...
# from models import Msats, Sequence, Combined, Primers

##### Models.py

from sqlalchemy import Column, Table, INTEGER, TEXT, FLOAT, ForeignKey, ForeignKeyConstraint, and_
...
class Sequence(Base):
    ...
    #msat = relationship('Msats', order_by='Msats.sequence_id',backref='sequence') #  backref='sequence'
...
class Msats(Base):
    ...
    sequence = relationship('Sequence', order_by='Sequence.id', backref='msats')
...
class Combined(Base):
    ...
    # define foreign key relationship
    sequence = relationship('Sequence', order_by='Sequence.id', backref='combined')
...
class Primers(Base):
    ...
    id = Column(u'id', INTEGER(), primary_key=True, nullable=False)
    sequence_id = Column(u'sequence_id', INTEGER(), nullable=False) # ForeignKey(u'combined.sequence_id')
    combined_id = Column(u'combined_id', INTEGER(), nullable=False) # ForeignKey(u'combined.id'),
    ...
    ForeignKeyConstraint(['sequence_id', 'combined_id'], ['combined.sequence_id', 'combined.id'])
    
    # relationship
    combined = relationship('Combined', backref='primers2', primaryjoin=(combined_id == Combined.id), foreign_keys=[sequence_id, combined_id] )
...
class TaggedPrimers(Base):
    ...
    sequence_id = Column(u'sequence_id', INTEGER(),  nullable=False) # ForeignKey(u'combined.sequence_id'),
    id = Column(u'id', INTEGER(), nullable=False, primary_key=True) # ForeignKey(u'combined.id'),
    primer = Column(u'primer', INTEGER(), nullable=False)
    ...
    ForeignKeyConstraint(['sequence_id', 'id'], ['combined.sequence_id', 'combined.id'])
    
    
    # relationship    
    combined = relationship('Combined', backref='tagged_primers', primaryjoin=(id == Combined.id), foreign_keys=[sequence_id, id] )
...
class CombinedComponents(Base):
    ...
    sequence_id = Column(u'sequence_id', INTEGER(), nullable=False, primary_key=True) # ForeignKey(u'combined.sequence_id')
    combined_id = Column(u'combined_id', INTEGER(), nullable=False) #
    motif = Column(u'motif', TEXT(length=None, convert_unicode=False, assert_unicode=None,\
                    unicode_error=None, _warn_on_bytestring=False))
    ...
    combined = relationship('Combined', backref='combined_components',\
                primaryjoin=(combined_id == Combined.id ),\
                foreign_keys=[sequence_id, combined_id] )
...
def run_query(msat_motif=None, msat_motif_size=0, msat_motif_count=0, combine_loci=False, design_primers=False, tag_primers=False):
...
z = db_session.query(CombinedComponents).filter(CombinedComponents.combined_id == 0).filter(CombinedComponents.sequence_id == 5)
print z

for item in z:
    print item.motif, item.length, item.combined.members
